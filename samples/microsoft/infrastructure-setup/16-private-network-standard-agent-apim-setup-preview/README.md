---
description: This set of templates demonstrates how to set up Azure AI Agent Service with virtual network isolation with private network links to connect the agent to your secure data, including Azure API Management integration for enhanced API gateway capabilities.
page_type: sample
products:
- azure
...
# Azure AI Agent Service: Standard Agent Setup with E2E Network Isolation and API Management (Preview)
...
---
## Overview
This infrastructure-as-code (IaC) solution deploys a network-secured Azure AI agent environment with private networking, role-based access control (RBAC), and Azure API Management integration for enhanced API gateway capabilities.

Standard setup supports private network isolation through utilizing **Bring Your Own Virtual Network (BYO VNet)** approach, also known as **custom VNet support with subnet delegation.** This preview version additionally includes **Azure API Management (APIM)** to provide advanced API gateway features, request/response transformation, throttling, authentication, and monitoring capabilities for your AI agent deployments. 
...
**API Management Integration (Preview)**
- **Optional Integration**: This template supports integration with an existing Azure API Management service to provide enhanced API gateway capabilities
- **Features Enabled**: When APIM is integrated, it provides request/response transformation, throttling, authentication, monitoring, and centralized API management
- **Private Connectivity**: If provided, APIM will be connected via private endpoint to ensure secure communication within your network perimeter
- **Resource Requirements**: You must provide an existing APIM service resource ID; the template does not create APIM resources automatically 
